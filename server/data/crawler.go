...
func main() {
	...
	pokemons := extractPokedex(doc)
	var allPoke []Pokedex
	var allPokeInfo PokeInfo

	for _, poke := range pokemons {

		allPokeInfo = fetchDetailInfo(poke.Link)
...
func extractPokedex(n *html.Node) []Pokedex {
	var pokemon []Pokedex
		...
		if n.Type == html.ElementNode && n.Data == "span" {
			for _, attr := range n.Attr {
				if attr.Key == "class" && attr.Val == "infocard-lg-data text-muted" {
					PokeId := strings.Split(extractOnce(n, "small"), "a")[0]
					PokeId = strings.TrimSpace(PokeId)
					currentPoke.Id = PokeId
					pokeName := extractInsideTag(n, "a", "class", "ent-name")
					pokeName = strings.TrimSpace(pokeName)
					currentPoke.Name = pokeName
					links := extractStringElement(n, "a", "href")
					currentPoke.Link = strings.Split(links, "/type")[0]
					types := extractInsideTag(n, "a", "class", "itype")
...
func fetchDetailInfo(url string) PokeInfo {
		...
		if n.Type == html.ElementNode && n.Data == "table" {
			for _, attr := range n.Attr {
				if attr.Key == "class" && attr.Val == "vitals-table" {
					result := extractOnce(n, "th")
					stats := strings.Split(result, " ")
					for _, st := range stats {
						if st == "HP" {
							listStat := extractStatNumber(n, "td", "class", "cell-num")
		...
		if n.Type == html.ElementNode && n.Data == "table" {
			for _, attr := range n.Attr {
				if attr.Key == "class" && attr.Val == "type-table type-table-pokedex" {
					result := extractOnce(n, "a")
					stats := strings.Split(result, " ")
					for _, st := range stats {
						if st == "Nor" {
							listDefRate := extractRatioDef(n)
...
func extractInsideTag(n *html.Node, data, key, val string) string {
	var result = ""
	...
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		result += extractInsideTag(c, data, key, val)
	}
	return result
}
func extractOnce(n *html.Node, tagName string) string {
	var result string
	...
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		result += extractOnce(c, tagName)
	}
	return result
}
func extractStringElement(n *html.Node, data, key string) string {
	var result string
	if n.Type == html.ElementNode && n.Data == data {
		for _, attr := range n.Attr {
			if attr.Key == key {
	...
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		result += extractStringElement(c, data, key)
	}
	return result
}
func extractStatNumber(n *html.Node, tagName, key, val string) []int {
	var numbers []int
	if n.Type == html.ElementNode && n.Data == tagName {
		for _, attr := range n.Attr {
	...
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		number := extractStatNumber(c, tagName, key, val)
...
func extractRatioDef(n *html.Node) []int {
	var percentg []int
